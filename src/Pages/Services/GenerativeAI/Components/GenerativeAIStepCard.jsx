import GenerativeAIStepListItem from "./GenerativeAIStepListItem";

export default function GenerativeAIStepCard({ number, title, items, delay }) {
  return (
    <div
      ...
      <div className="relative space-y-4">
        ...
        <ul className="space-y-3">
          {items.map((item, index) => (
            <GenerativeAIStepListItem key={index} text={item} />
          ))}
        </ul>
      </div>
    </div>
